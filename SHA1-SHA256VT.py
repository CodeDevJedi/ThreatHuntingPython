"""
VirusTotal Hash Metadata Query
------------------------------
Looks up a list of hashes using the VirusTotal API and writes out relevant details and file names.

Instructions:
- Prepare a local text file with one hash per line (MD5/SHA1/SHA256).
- Supply your VirusTotal API key at runtime or as the VT_API_KEY environment variable.
- Outputs:
    - vt_details.txt: Full VirusTotal results per hash.
    - files.txt: All distinct 'names' fields from results.
    - output_sha256.txt: SHA256 values for each hash found.
...
def read_hashes(filename):
    """Read hashes from a file, one per line."""
    try:
